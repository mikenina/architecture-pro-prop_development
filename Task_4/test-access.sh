...
for user in "$USER_DEVOPS" "$USER_SECURITY" "$USER_MONITORING"; do
  for namespace in "$DEV_NAMESPACE" "$MONITORING_NAMESPACE"; do
    ...
    if [ -f "$kubeconfig" ]; then
        echo -n "$user: "
        if kubectl --kubeconfig="$kubeconfig" auth can-i delete pod -n $namespace 2>/dev/null | grep -q "yes"; then
            echo "✓ delete pod - Доступ есть в ${namespace}"
        else
            echo "✗ delete pod - Доступа нет в ${namespace}"
        fi
    else
        echo "$user: Kubeconfig не найден"
    fi
